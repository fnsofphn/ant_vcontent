...
export default async function handler(req, res) {
  ...
  const bucket = process.env.SUPABASE_WORKFLOW_BUCKET || process.env.SUPABASE_INTAKE_BUCKET || 'vcontent-intake';
  ...
  try {
    ...
    const moduleCode = String(body.module || '').trim();
    ...
    if (!orderId || !productId || !moduleCode || !stageCode || !slot || !fileName || !base64Data) {
    ...
    const objectPath = [
      'workflow',
      slugify(moduleCode) || 'unknown-module',
      slugify(stageCode) || 'unknown-stage',
      slugify(orderId) || 'unknown-order',
      slugify(productId) || 'unknown-product',
      slugify(slot) || 'unknown-slot',
      safeFileName,
    ].join('/');

    const previousObjectPath = extractStorageObject(previousUrl, bucket);
    if (previousObjectPath) {
      await admin.storage.from(bucket).remove([previousObjectPath]);
    }
